...
/// 从 UI/Emblem 解密 AB 中导出 PNG。
///
/// 流程:
/// 1. 扫描 variants/Chs/decrypted/UI/Emblem/utx_tex_em_*.ab
/// 2. SHA256 增量跳过（缓存 .hashes.json）
/// 3. AssetStudio tex2d 导出
/// 4. 重命名为 {emblem_id}.png → exports/emblems/
fn extract_emblems(data_dir: &Path, asset_studio_path: &Path) -> anyhow::Result<()> {
    use sha2::Digest;
    use std::collections::BTreeMap;
    use std::io::Read;
    ...
    let source_dir = data_dir
        ...
        .join("Chs")
        .join("decrypted")
        .join(EMBLEM_SOURCE_DIR);

    if !source_dir.exists() {
        anyhow::bail!(
            "徽章源目录不存在: {}（请先运行 wbu asset batch -v Chs）",
            source_dir.display()
        );
    }

    let output_dir = data_dir.join("exports").join("emblems");
    std::fs::create_dir_all(&output_dir)?;

    // 加载哈希缓存（emblem_id → sha256）
    let hash_cache_path = output_dir.join(".hashes.json");
    let hash_cache: BTreeMap<String, String> = if hash_cache_path.exists() {
        let raw = std::fs::read_to_string(&hash_cache_path)?;
        serde_json::from_str(&raw).unwrap_or_default()
    ...
    let bundles: Vec<_> = std::fs::read_dir(&source_dir)?
        ...
        .filter(|e| {
            let name = e.file_name().to_string_lossy().to_string();
            name.starts_with("utx_tex_em_") && name.ends_with(".ab")
        })
        .collect();

    if bundles.is_empty() {
        println!("UI/Emblem 目录为空，跳过");
        return Ok(());
    }
    ...
    for entry in &bundles {
        let name = entry.file_name().to_string_lossy().to_string();
        // utx_tex_em_100111300.ab → 100111300
        let id = name
            .strip_prefix("utx_tex_em_")
            .unwrap_or(&name)
            .strip_suffix(".ab")
            .unwrap_or(&name)
            .to_string();
    ...
    if stale_ids.is_empty() {
        println!("emblems 全部为最新（{} 个），跳过", bundles.len());
        let json = serde_json::to_string_pretty(&current_hashes)?;
        std::fs::write(&hash_cache_path, json)?;
        return Ok(());
    }
    ...
    let spinner = ProgressBar::new_spinner();
    spinner.set_style(
        ProgressStyle::with_template("{spinner} AssetStudio 导出徽章中... {elapsed}").unwrap(),
    );
    spinner.enable_steady_tick(std::time::Duration::from_millis(100));
