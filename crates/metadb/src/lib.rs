...
pub fn derive_final_key(key_b64: &str, base_key_b64: &str) -> anyhow::Result<Vec<u8>> {
    let k = base64::engine::general_purpose::STANDARD
        .decode(key_b64)
        .context("key b64 fail")?;
    let bk = base64::engine::general_purpose::STANDARD
        .decode(base_key_b64)
        .context("base_key b64 fail")?;
    Ok(k.iter()
        .enumerate()
        .map(|(i, &x)| x ^ bk[i % BASE_KEY_PERIOD])
        .collect())
}
...
pub fn decrypt_metadb(
    ...
    if key_b64.is_empty() {
        return Err(anyhow!("sqlite3mc_key must be set"));
    }
